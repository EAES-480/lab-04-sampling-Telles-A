...
df %>% summarise(
  n = n(),
  n_missing = sum(is.na(response_var)),
...
> I chose air temperature for my response variable because it is a simple environmental process to understand. Usually, it is colder at night and warmer during the day so I expect to see that pattern. For seasonality, because it the data is taken from Chicago, I would expect to see lower temperatures during the winter/fall months and higher temperatures during the spring/summer. 
...
|----------|-------|-------------|----------------------------|
|gpp      | µmolCO2 m-2 s-1 |Gross Primary Productivity|seasonal and daily cycle|
|ta       |Celcious|air temperature|seasonal and daily cycle|
|reco     |µmolCO2 m-2 s-1|Ecosystem Respiration|seasonal and daily cycle|
|nee      |µmolCO2 m-2 s-1|Net Ecosystem Exchange|seasonal and daily cycle|
|swc      |percentage|Soil water content (volumetric), range 0-100|seasonal cycle(more melt and precipitation in spring)|
...
> As expected, there are lower temperatures in January, then begin to rise closer to July, and drop again after October. There is also a daily cycle pattern that can be seen from each individual day. There seems to be a gap in the data before July 2023 that could be from a coding error. 
---
...
> The distribution is bimodal with two different peaks in air temperature. 
---
...
> The month grouping variable seems to explain more variability in my response because the distribution of boxplots matches the population distribution. It also explains the subtle differences in air temperature compared to day and night. 
...
> The mean and standard deviation of the one-sample estimate is very close to the population. This is not surprising because this sample is supposed to be a representation of the population.
...
> The sampling distribution does seem centered on the population mean. However, when you increase the number of samples, the distribution appears more normal, still approximately centered on the population mean.
...
> The SRS was a better approximation for the mean and standard deviation for this response variable. Though stratifying by month seemed reasonable because it explained the variability of the response variable, the SRS was closer to the population mean and standard deviation. 
...
> Seasonality is important for sampling because within the environment, changes in seasons can account for a lot of variability within a system. If sampling ignores latent grouping variables, there can be parts of the data that remains unexplained. In EaES, stratification is essential when there are distinct sub-groups that need to be represented. However, with stratified samplinga, one trade-off is the complexity it adds to a data set (vs. simple random sampling). 
...
> The systematic sampling approximated the population mean better but there was a larger standard deviation compared to SRS. The chosen k (48), may be interacting with a daily cycle because every 48 records is equivalent to about a day.  
...
> Cluster sampling may have a higher variance than SRS because it places samples into specific groups, meaning any deviation from that could create fluctuation within the data set. The seasonality of the time series data is relevant here because most months have similar air temperatures. Therefore, when you cluster by days that resemble about a month 
...
> The quasi-continuous sampling does preserve the daily pattern because the population time series matches the quasi-continuous time series. This could mean that it would also have the same seasonality pattern, especially with the close approximation of the mean and standard deviation. This form is preferable over sparse discrete sampling when there is a relationship that has high variability.
...
>  According to the table, the best estimate of the population mean was from the systematic sampling, however quasi-continuous was a close second. This is because the k value chosen was the aligned with the pattern of the daily cycle. The best estimate for the population standard deviation was from the quasi-continuous sampling. In a real EaES study, if there were limited field days and I already knew of an apparent pattern like the seasonal or daily cycles, the hybrid strategy I would propose would be stratified and simple random sampling. With these two sampling strategies, I could stratify by month and then do the simple random sampling to avoid bias. 
